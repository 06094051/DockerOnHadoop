...
public class ClientRMService extends AbstractService implements
  ...
  @Override
  public UpdateTBUrlResponse updateApplicationTBUrl(UpdateTBUrlRequest request) throws YarnException, IOException {
    ApplicationId applicationId = request.getApplicationId();
    ...
    if (application == null) {
      RMAuditLogger.logFailure(callerUGI.getUserName(),
          AuditConstants.UPDATE_TBURL_REQUEST, "UNKNOWN", "ClientRMService",
          "Trying to update tensorboard an absent application", applicationId);
      throw new ApplicationNotFoundException("Trying to update tensorboard url an absent"
          + " application " + applicationId);
    }
    ...
    if (!checkAccess(callerUGI, application.getUser(),
        ApplicationAccessType.MODIFY_APP, application)) {
      RMAuditLogger.logFailure(callerUGI.getShortUserName(),
          AuditConstants.UPDATE_TBURL_REQUEST,
          "User doesn't have permissions to "
              + ApplicationAccessType.MODIFY_APP.toString(), "ClientRMService",
          AuditConstants.UNAUTHORIZED_USER, applicationId);
      throw RPCUtil.getRemoteException(new AccessControlException("User "
    ...
    this.rmContext.getDispatcher().getEventHandler().handle(
        new RMAppAttemptStatusupdateEvent(application.getCurrentAppAttempt().getAppAttemptId(),  request.getTensorboardUrl()));

    return UpdateTBUrlResponse.newInstance(applicationId, application.getTensorboardUrl());
  }
