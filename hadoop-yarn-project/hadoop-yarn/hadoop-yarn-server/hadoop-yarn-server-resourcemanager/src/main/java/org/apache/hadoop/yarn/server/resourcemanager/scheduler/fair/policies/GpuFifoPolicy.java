...
@Private
@Unstable
public class GpuFifoPolicy extends SchedulingPolicy {
  @VisibleForTesting
  public static final String NAME = "gpufifo";
  ...
  static class FifoComparator implements Comparator<Schedulable>, Serializable {
    ...
    public int compare(Schedulable s1, Schedulable s2) {
      ...
      // 资源分配好的放在队尾， 在队列中 skipNumApp 会根据队列顺序去跳过前面不能分配的任务来分配后续的任务
      if(res > 0 && (s2 instanceof FSAppAttempt) && ((FSAppAttempt) s2).isResourceReady()){
        return -1;
      }
      if(res < 0 && (s1 instanceof FSAppAttempt) && ((FSAppAttempt) s1).isResourceReady()){
        return 1;
      }
      return res;
    }
  ...
  public void computeUsage(Resource usage, List<FSAppAttempt> runnableApps){
    for (FSAppAttempt app : runnableApps) {
      // 为了防止多队列的时候，多个队列的一个app 都是分了一部分资源，计算usage只计算那些app资源都分配完的任务
      if(app.isResourceReady())
